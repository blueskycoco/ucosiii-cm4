...
static  OS_MUTEX   cm_ptr;

//LWEVENT_STRUCT lwevent_buffer_queued[MCC_MQX_LWEVENT_COMPONENTS_COUNT];
//LWEVENT_STRUCT lwevent_buffer_freed;
static  OS_MUTEX lwevent_buffer_queued[MCC_MQX_LWEVENT_COMPONENTS_COUNT];
static  OS_MUTEX lwevent_buffer_freed;
...
static void mcc_cpu_to_cpu_isr(void *param)
{
    ...
    if(DEF_OK == BSP_OS_SemWait(&cm_ptr, 0)) {
...
int mcc_init_os_sync(void)
{
    ...
    for(i=0; i<MCC_MQX_LWEVENT_COMPONENTS_COUNT; i++) {
		OSSemCreate(&lwevent_buffer_queued[i],
                "Trace Lock",
                 i,
                &os_err);
        if(OS_ERR_NONE != os_err) {
	...
	OSSemCreate(&lwevent_buffer_freed,
                "buf Lock",
                 1u,
                &os_err);
    if(OS_ERR_NONE != os_err) {
