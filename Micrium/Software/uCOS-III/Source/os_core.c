...
void  OSInit (OS_ERR  *p_err)
{
    ...
    OS_AppTaskCreateHookPtr = DEF_NULL;                         /* Clear application hook pointers                      */
    ...
    if ((OS_CFG_PRIO_MAX >= (OS_PRIO)-1) ||
        (OS_CFG_PRIO_MAX <  8u)) {
       *p_err = OS_ERR_PRIO_MAX_INVALID;
    }
...
*                             OS_ERR_OS_NOT_INIT     OS is not initialized, OSStart() has not effect
*                             OS_ERR_OS_NO_APP_TASK  No application task created, OSStart() has not effect
*
* Returns    : none
*
