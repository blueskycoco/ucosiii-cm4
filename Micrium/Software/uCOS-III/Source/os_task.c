...
* Returns    : none
*
* Note(s)    : 1) OSTaskCreate() will return with the error OS_ERR_STK_SIZE_INVALID when a stack overflow is detected
*                 during stack initialization. In that specific case some memory may have been corrupted. It is
*                 therefore recommended to treat OS_ERR_STK_SIZE_INVALID as a fatal error.
************************************************************************************************************************
*/
...
#if (CPU_CFG_STK_GROWTH == CPU_STK_GROWTH_HI_TO_LO)
    for (i = 0u; i < 8u; i++) {
        *(p_stk_base + i) = 0xDEADBEEF;
    }
#else
    for (i = 0u; i < 8u; i++) {
        *(p_stk_base + stk_size - i) = 0xDEADBEEF;
    }
#endif
#endif
...
CPU_BOOLEAN  OSTaskStkRedzoneChk (OS_TCB  *p_tcb)
{
    ...
    for (i = 0u; i < 8u; i++) {
        if (*p_sp != (CPU_DATA)0xDEADBEEFu) {
            return (DEF_FAIL);
        }
        p_sp++;
    }
#else
    p_sp = p_tcb->StkBasePtr + p_tcb->StkSize;
    for (i = 0u; i < 8u; i++) {
        if (*p_sp != (CPU_DATA)0xDEADBEEFu) {
            return (DEF_FAIL);
        }
